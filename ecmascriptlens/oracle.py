...
"""Oracles: pure-function probes, check-line verdicts, lib-gate readings.

Every expected value in every suite was measured by running one of these
three instruments. Nothing here judges; everything measures. Requires node
on PATH (checker lib via RDC_LIB) for purefn/check; gate readings run the
ai-slop gate from the same tree.
"""
...
def purefn(fn, arg):
    """Run a pure function of lib/dtd.mjs. Returns the measured output."""
    if fn not in PURE_FNS:
        raise ValueError(f"unknown fn {fn}")
    _rc, doc = _node(os.path.join(VENDOR, "ecma_probe.mjs"), fn, json.dumps(arg))
    if not doc.get("ok"):
